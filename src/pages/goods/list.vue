  ...
  <div>
    ...
    <el-card shadow="never" class="border-0">
      ...
      <list-header layout="create,refresh" @create="handleCreate" @refresh="getData">
        <el-popconfirm v-if="searchForm.tab !== 'delete'" title="是否要批量删除该商品？" confirmButtonText="确认"
                       cancelButtonText="取消"
                       @confirm="handleMutiDelete">
          <template #reference>
        ...
        <el-button size="small" type="success" plain @click="handleMultiStatusChange(1)"
                   v-if="searchForm.tab === 'all' || searchForm.tab === 'off'">上架
        </el-button>
        <el-button size="small" type="warning" plain @click="handleMultiStatusChange(0)"
        ...
        <el-table-column label="操作" align="center" width="400">
          <template #default="scope">
            <div v-if="searchForm.tab !== 'delete'">
              <el-button class="btn_text px-1" type="primary" size="small" text @click="handleEdit(scope.row)">修改
              </el-button>
              <el-button
                  :type=" (scope.row.sku_type === 0 && !scope.row.sku_value) || (scope.row.sku_type === 1 && !scope.row.goods_skus.length) ? 'danger' : 'success'"
                  @click="handleSetGoodsSkus(scope.row)" class="btn_text px-1" type="primary" size="small" text>
                商品规格
              </el-button>
              <el-button class="px-1" :type="scope.row.goods_banner.length === 0 ? 'danger' : 'success'" size="small"
                         text @click="handleSetGoodsBanners(scope.row)" :loading="scope.row.bannersLoading">设置轮播图
              </el-button>
              <el-button class="px-1" :type="!scope.row.content ? 'danger' : 'success'" size="small" text
              ...
              <el-popconfirm title="是否要删除该商品？" confirmButtonText="确认" cancelButtonText="取消"
                ...
                <template #reference>
                  <el-button class=" btn_text px-1" text type="primary" size="small">删除</el-button>
                </template>
              </el-popconfirm>
            </div>
      ...
      <form-drawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
        <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
          ...
          <el-form-item label="封面" prop="cover">
            <choose-image v-model="form.cover"/>
          </el-form-item>
    ...
    <banners @reloadData="getData" ref="bannersRef"/>
    <content @reloadData="getData" ref="contentRef"/>
    <skus @reloadData="getData" ref="skusRef"/>
  </div>
</template>
<script setup>
